...
class Consistent : AppCompatActivity(), View.OnClickListener {
    ...
    override fun onClick(view: View) {
        ...
        when (view.id) {
            R.id.reviews_button, R.id.reviews_tw -> {
                newActivity = Intent(this@Consistent, Reviews::class.java)
                newActivity.putExtra("type", "Последовательно-постоянный")
                startActivity(newActivity)
            }
            R.id.place_an_order, R.id.place_an_order_tw -> {
                newActivity = Intent(this@Consistent, MakingAnOrder::class.java)
                newActivity.putExtra("type", "Последовательно-постоянный")
                startActivity(newActivity)
            }
        }
